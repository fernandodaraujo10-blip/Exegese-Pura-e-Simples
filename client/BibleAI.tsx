...
const SUGGESTIONS: Record<AIMode, string[]> = {
  COUNSELOR: [
    "Como encontrar descanso em Cristo em meio à ansiedade?",
    "A doutrina da Providência em tempos de luto",
    "Biblicamente, como lidar com o pecado recorrente?",
    "O perdão bíblico vs reconciliação",
    "Contentamento cristão em uma era consumista"
  ],
  THEOLOGIAN: [
    "Impacto do Pacto da Redenção na salvação",
    "Comunicação de Atributos na Pessoa de Cristo",
    "Justificação por Sola Fide vs Tiago 2",
    "Relação entre Lei e Evangelho na tradição clássica",
    "A Doutrina da Inspiração Plenária e Verbal"
  ],
  STUDIES: [
    "Sensus Plenior no Antigo Testamento",
    "Tipologia de Cristo no Tabernáculo",
    "Estrutura Aliancista de Gênesis 3:15",
    "Análise do termo 'Logos' no Prólogo de João",
    "Contexto histórico do Sínodo de Dort"
  ]
};
        ...
        <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-lg">
          {Object.values(TheologyLine).map(t => (
            <button
              ...
              className={`flex-1 py-1.5 text-[9px] font-bold uppercase tracking-wider rounded-md transition-all ${theology === t ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-400'
                }`}
            >
        ...
        <div className="w-full overflow-x-auto whitespace-nowrap px-4 py-3 bg-gray-50 border-b border-gray-100 hide-scrollbar flex gap-2">
          {SUGGESTIONS[activeMode].map((sug, i) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-[11px] font-medium border transition-colors shrink-0 ${activeMode === 'COUNSELOR' ? 'bg-rose-50 text-rose-600 border-rose-100 hover:bg-rose-100' :
        ...
        <div className="p-3 bg-white">
          <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-xl border border-gray-200 focus-within:border-indigo-500 transition-all">
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-all ${!input.trim() || isLoading ? 'text-gray-300' : 'text-indigo-600 bg-indigo-50'
                }`}
            >
              <Send size={20} />
            </button>
